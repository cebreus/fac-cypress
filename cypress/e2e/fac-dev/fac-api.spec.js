describe('Test FAC API', () => {
  const root = 'https://apidev.fastandcomfy.io';
  const rootSwagger = 'https://developersdev.fastandcomfy.io/swagger';
  const email = 'me@cebre.us';
  ...
  it('Send form', () => {
    cy.get('[data-cy="FormGetApiKey"]')
      ...
      .get('#email')
      .then(($input) => {
        cy.wrap($input).focus().type(email);
      })
      .get('button[type="submit"]')
      .then(($btn) => {
